...
?>
    ...
    <strong><span class="badge"><?php echo "Nº OS: ".$objPedido->getCodigo(); ?></span></strong> <strong>Cliente: </strong><?php echo $primeiroNome." ".$ultimoNome; ;?> | <strong>Usuário:</strong> <?php echo $objPedido->getUsuario()->getLogin(); ?> | <strong>Data Emissão:</strong> <?php echo $objDateFormat->date_format($objPedido->getData_inicio()); ?> | <strong>Data de Entrega:</strong> <?php echo $objDateFormat->date_format($objPedido->getData_final()); ?> | <!--<strong>Status:</strong> <strong><?php echo $objPedido->printStatus(); ?></strong>-->
...
<div class="panel panel-primary">
  ...
  <div class="panel-body" style="background-color:#FFFAFA; ">
   ...
   <?php echo form_open('pedidos/add_item/'.$objPedido->getId_pedido(),array("onsubmit"=>"return validate()","class"=>"form-horizontal","id"=>"formulario")); ?>
    ...
    <!--<label class="radio-inline">
    <input type="radio" name="tipo_item" id="selecao_manual" value="3"> Informar Manualmente
    </label>
    -->
  ...
  <div class="col-xs-5" id="manual" style="display:none;">
   ...
   <div class="col-xs-4" id="camada_nome_produto">
    ...
    <select style="width:400px;" name="id_produto" id="id_produto" class="form-control select2_single" tabindex="1">
      ...
      <option value="<?php echo $objProduto->getId_produto(); ?>" <?php echo set_select('id_produto'); ?>>
        <?php echo $objProduto->getCodigo()." ".$objProduto->getDescricao()." R$: ".$objProduto->getValor_venda(); ?>
      </option>
              <?php endforeach; ?>
    </select>
    ...
    <div class="panel-body">
      <div class="row">
        <div class="col-xs-2">
          ...
          <input type="text" tipo="moneyReal" class="form-control" id="desconto" tipo="moneyReal" value="<?php echo set_value('desconto',number_format($objPedido->getDesconto_perc(), 2, ',', ''))?>" >
        </div>
        <div class="col-xs-2">
...
$(function () {
    ...
    //$('.total_pag_dinheiro').val(total_pagar.toFixed(2).replace(".", ","));
    
    //$('.botao_total_pedido').text("TOTAL PEDIDO = R$: "+total_pagar.toFixed(2).replace(".", ","));

    //$('.botao_total_saldo').text("SALDO = R$: "+total_final.toFixed(2).replace(".", ","));
    ...
    //DESCONTO
     $("#desconto").keyup(function(){
       ...
       /*else{
          ...
          alert('Valor de desconto maior que o valor da venda');
          location.reload();
