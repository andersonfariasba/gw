...
?>
  ...
  <li><a href="<?php echo site_url('locacao/alterar_status/'.$objPedido->getId_pedido().'/'.$objStatus->getId_status()); ?>"><i class="fa fa-check"></i> <strong><?php echo strtoupper($objStatus->getStatus()); ?></strong></a></li>
  <?php endforeach; ?>
    ...
    <!--<button type="button" class="btn btn-sm btn-primary"><strong><i class="glyphicon glyphicon-print" aria-hidden="true" title="Imprimir Pedido"></i></strong></button>--> 
      <a href="<?php echo site_url('locacao/imprimir/'.$objPedido->getId_pedido()); ?>" target="_blank" class="btn btn-sm btn-primary"><strong><i class="glyphicon glyphicon-print" aria-hidden="true" title="Imprimir Pedido"></i></strong></a>
...
<div class="panel panel-primary">
  ...
  <div class="panel-body" style="background-color:#FFFAFA; ">
   ...
   <?php echo form_open('locacao/add_item/'.$objPedido->getId_pedido(),array("onsubmit"=>"return validate()","class"=>"form-horizontal","id"=>"formulario")); ?>
...
<div class="row">
  ...
  <label>PERÍODO DE:</label>
    <input type="text" name="data_inicio" id="data_inicio" class="form-control calendario">
  </div>

  <div class="col-xs-2">
  <label>ATÉ</label>
    <input type="text" name="data_prev_entrega" id="data_fim" class="form-control calendario">
  </div>
  
  <div class="col-xs-1">
  <label>QTD</label>
    <!-- tipo="moneyReal" -->
    <input type="text" name="qtd" onkeypress='return SomenteNumero(event)'name="parcela_qtd" class="form-control" id="prazo_estimado">
  </div>
   ...
   <?php if($objPedido->getFaturado()!=SIM){ ?>
   <button type="submit" class="btn btn-primary"><strong><i class="glyphicon glyphicon-plus-sign" aria-hidden="true"></i></strong></button>
  <?php } 
      ...
      <?php 
         ...
         foreach($listItens as $objItem): 
            ...
            $total = $total + $sub_total; ?>
                ...
                <td>
                  <?php echo $objDateFormat->date_format($objItem->getData_inicio())." - ".$objDateFormat->date_format($objItem->getData_prev_entrega()); ?>
                </td>
...
<div id="myModal" class="modal fade">
  ...
  <option value="<?php echo $objUser->getId_usuario(); ?>" <?php echo set_select('id_usuario',$user,$objPedido->usuarioIs($user)); ?>>
          ...
          <div class="form-group">
            <label for="recipient-name" class="control-label">Data Validade Cotação:</label>
             <input type="text" name="data_validade" class="form-control calendario" value="<?php echo $objDateFormat->date_format($objPedido->getData_validade()); ?>">
          </div>


           <div class="form-group">
            <label for="recipient-name" class="control-label">Data Finalização:</label>
    ...
    <select style="width:350px;" name="edit_id_produto" id="edit_id_produto" class="form-control select2_single" tabindex="1">
      ...
      <?php } ?>
          ...
          <div class="form-group">
            ...
            <div class="form-group">
            <label for="recipient-name" class="control-label">Data Inicio:</label>
             <input type="text" name="edit_data_inicio" id="edit_data_inicio"  class="form-control calendario">
          </div>

             <div class="form-group">
            <label for="recipient-name" class="control-label">Data Entrega:</label>
             <input type="text" name="edit_data_prev_entrega" id="edit_data_prev_entrega"  class="form-control calendario">
          </div>
    ...
    e.preventDefault();
     ...
     $.ajax({
             ...
             /* if(escopo==1){ //caso seja pedido pelo estoque
              ...
              window.location.href="<?php echo site_url('locacao/imprimir'); ?>/"+id_pedido;
              window.opener.location.href="<?php echo site_url('locacao/filtro/2'); ?>";
...
$('#add_cotacao_btn').click(function(e){
    ...
    e.preventDefault();
     ...
     $.ajax({
             type: 'POST',
             url: "<?php echo site_url('locacao/salvar_cotacao/'); ?>",         
            ...
            // setTimeout('window.location.href = "<?php echo site_url('pedidos/solicitar_cliente2/2'); ?>"; window.opener.location.href="<?php echo site_url('pedidos/filtro/1'); ?>"' , 3000);
              ...
              //window.location.href="<?php echo site_url('locacao/solicitar_cliente/2'); ?>";
              window.opener.location.href="<?php echo site_url('locacao/filtro/1'); ?>";
              window.close();
...
$('#add_cotacao_venda_btn').click(function(e){
    ...
    e.preventDefault();
     ...
     $.ajax({
             type: 'POST',
             url: "<?php echo site_url('locacao/cotacao_venda/'); ?>",         
             data: $('#ajax_forma_cotacao_venda').serialize(),
             success : function(txt){
              var id_pedido = $('#id_pedido').val();
             
              window.location.href="<?php echo site_url('locacao/novo/'); ?>/"+id_pedido;
              window.opener.location.href="<?php echo site_url('locacao/filtro/2'); ?>";
...
//INICIO VERIFICAÇÃO 50%
              ...
              var valor_prazo = moedaParaNumero($('#valor_pago_forma').val());
               ...
               if(forma_prazo==9999 && (valor_prazo > valor_minimo)){
              //  if(forma_prazo==4){
...
$('#btnYes').click(function() {
    ...
    location.href="<?php echo site_url('locacao/excluir_item'); ?>/"+id+"/"+id_pedido;
 ...
 $('#id_produto').change(function(){
  ...
  $('#edit_pedido_btn').click(function(e){
    ...
    e.preventDefault();
     ...
     $.ajax({
             type: 'POST',
             url: "<?php echo site_url('locacao/editar_pedido/'); ?>",         
             ...
             success : function(txt){
              ...
              $('#myModal').modal('hide');
               ...
               var tipo = 2;
               window.location.href="<?php echo site_url('locacao/novo/'); ?>/"+id_pedido;
               window.opener.location.href="<?php echo site_url('locacao/filtro/'); ?>/"+tipo;
 ...
 $(".open-AddBookDialog").click(function(){
   ...
   $.getJSON(url, function(j){
       ...
       for (var i = 0; i < j.length; i++) {
          ...
          descricao = j[i].descricao;
          data_inicio = j[i].data_inicio;
          data_prev_entrega = j[i].data_prev_entrega;
      ...
      $("#edit_data_prev_entrega").val(data_prev_entrega);
...
function days() {
            var a = $("#data_inicio").datepicker('getDate').getTime(),
                b = $("#data_fim").datepicker('getDate').getTime(),
                c = 24*60*60*1000,
                diffDays = Math.round(Math.abs((a - b)/(c)));
            $("#prazo_estimado").val(diffDays)
}


$(document).ready(function () {
//$.datepicker.setDefaults({dateFormat: 'dd/mm/yy',minDate: +1,changeMonth: true,changeYear: true,numberOfMonths: 2,constrainInput:true,beforeShowDay:nationalDays,});
        $.datepicker.setDefaults({dateFormat: 'dd/mm/yy',});
    var selector = function (dateStr) {
            var d1 = $('#data_inicio').datepicker('getDate');
            var d2 = $('#data_fim').datepicker('getDate');
            var diff = 0;
            if (d1 && d2) {
                diff = Math.floor((d2.getTime() - d1.getTime()) / 86400000); // ms per day
            }
            $('#prazo_estimado').val(diff);
        }
$('#data_inicio').datepicker({
            onSelect: function(selectedDate) {
    var minDate = $(this).datepicker('getDate');
    //if (minDate) {minDate.setDate(minDate.getDate() + 0);}//min days requires
...
$('#data_fim').datepicker({
  ...
  var maxDate = $(this).datepicker('getDate');    
    ...
    $('#data_inicio,#data_fim').change(selector)
        });
