...
class Est_movimentacaoDao extends CI_Model {
    ...
    public function cadastrar($objMovimentacao){
        $sucess = $this->db->insert("est_movimentacao",$objMovimentacao->toArray());
        if(!$sucess){
     ...
     public function visualizarSimples($id_movimentacao){
        ...
        if($query->num_rows()>0){
            ...
            $objMovimentacao = $this->Factory->createPojo("est_movimentacao",$dados);
                ...
                //PRODUTO
                $produtoBussiness = $this->Factory->createBusiness("est_produtos");
    ...
    public function excluir($id_movimentacao){
        ...
        $sucess = $this->db->update("est_movimentacao",$dados);
         if(!$sucess){
             throw new Exception($this->db->_error_message(),$this->db->_error_number());
         }
    }
    ...
    public function filtro_financeiro($dados) {
     ...
     public function filtro_mov_total($dados) {
        ...
        $this->db->select('sum(est_movimentacao.qtd_mov) as qtd, est_movimentacao.tipo_movimentacao, est_produtos.descricao,est_produtos.valor_custo,est_produtos.valor_venda');
        $this->db->from("est_movimentacao");
        $this->db->join('est_produtos', 'est_produtos.id_produto = est_movimentacao.id_produto');
        $this->db->group_by("est_movimentacao.tipo_movimentacao");
         $this->db->group_by("est_movimentacao.id_produto");

         if($dados["tipo_movimentacao"]!=null){
          $this->db->where("est_movimentacao.tipo_movimentacao",$dados["tipo_movimentacao"]);
        }
        ...
        $data_ate = $dados["data_ate"];
         ...
         if ($data_de != NULL && $data_ate != NULL):
              ...
              $this->db->where("DATE(est_movimentacao.data) BETWEEN '$data_de' AND '$data_ate'");
         endif;
    ...
    public function total_item($id_pedido,$id_pedido_item) {
      ...
      $query = $this->db->query("select id_pedido,id_pedido_item,sum(qtd_mov) as qtd_mov from est_movimentacao where deletado = 0 and id_pedido = ".$id_pedido." and id_pedido_item = ".$id_pedido_item);
