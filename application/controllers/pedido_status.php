...
class Pedido_status extends MY_Controller {
...
<strong><i class="fa fa-check"></i></strong> ', '</div>');
    ...
    public function cadastrar($msg=null){
        ...
        if($this->form_validation->run()==FALSE){
            $content = $this->load->view("pedido_status/cadastrar",$info,TRUE);
            ...
            $categoriaBusiness = $this->Factory->createBusiness("fin_status_pedido");
            $cod_categoria = $categoriaBusiness->cadastrar($dados);
            $msg = true;
            redirect('pedido_status/cadastrar/'.$msg);
        }
    }
    ...
    public function filtro(){
        try {
            ...
            $content = $this->load->view("pedido_status/filtro",$info,TRUE);
            $this->loadPage($content);
   ...
   //EDIÇÃO
      public function editar($id_status,$msg=null){
          ...
          if($this->form_validation->run()==FALSE){
              ...
              $content = $this->load->view("pedido_status/editar",$info,TRUE);
              $this->loadPage($content);
            ...
            $msg = true;
            redirect('pedido_status/editar/'.$dados['id_status'].'/'.$msg);
           }
      }
   ...
   //VISUALIZAÇÃO
    public function visualizar($id_status){
          try {
              ...
              $content = $this->load->view("pedido_status/visualizar",$info,TRUE);
              $this->loadPage($content);
