...
class Pedidos_mesa extends MY_Controller {
	
    //VALIDAÇÃO
    private function Rules(){
        $this->form_validation->set_rules('id_mesa','Mesa','required');
    ...
    $this->load->helper(array('form','url'));
      ...
      $controleBusiness = $this->Factory->createBusiness("com_mesas");
      $listControle = $controleBusiness->controle_mesas(null);
      $info["listControle"] = $listControle;
  
    $content = $this->load->view("pedidos_mesa/controle",$info,TRUE);
    $this->loadPage($content);
     ...
     //CADASTRAR
    public function abrir($id_mesa,$msg=null){
        $this->load->helper(array('form','url'));
        ...
        if($this->form_validation->run()==FALSE){
          ...
          $this->load->view("pedidos_mesa/abrir",$info);
          ...
          if($dados['id_garcon']==""){
              $dados['id_garcon'] = NULL;
          }


          $cod_pedido = $pedidosBusiness->cadastrar($dados);

          redirect('pedidos_mesa/novo/'.$cod_pedido);
    ...
    public function solicitar_cliente($tipo,$id_cliente=null){
        ...
        $this->load->library('form_validation');
          ...
          redirect('pedidos/novo/'.$cod_pedido);
    ...
    public function filtro($tipo,$aba=null,$dados=null){
        try {
            ...
            //Pedidos
            $pedidosBusiness = $this->Factory->createBusiness("com_pedidos");
            $listPedido = $pedidosBusiness->filtro_bar($tipo);
          ...
          $info['listColaborador'] = $listColaborador;
            ...
            $content = $this->load->view("pedidos_mesa/filtro",$info,TRUE);
            $this->loadPage($content);
            ...
            //Pedidos
            $pedidosBusiness = $this->Factory->createBusiness("com_pedidos");
            $listPedido = $pedidosBusiness->filtro_bar($tipo,$dados);
            $info["listPedidos"] = $listPedido;
   ...
   function novo($id){
    ...
    //PEDIDO
    $pedidosBusiness = $this->Factory->createBusiness("com_pedidos");
    $objPedido = $pedidosBusiness->visualizar_bar($id);
  ...
  //Listagem de Clientes
    ...
    $this->load->view("pedidos_mesa/novo",$info);
           ...
           //Pegar valor do produto
            ...
            $objPedido = $pedidoBusiness->visualizar($dadosForm['id_pedido']);
              ...
              $dados['id_cliente'] = $dadosForm['id_cliente']; 
               $dados['id_garcon'] = $dadosForm['id_garcon']; 
         ...
         //  $content = $this->load->view("pedidos/visualizar",$info,TRUE);
          // $this->loadPage($content);
           ...
           $itemBusiness = $this->Factory->createBusiness("com_pedidos_itens");
            ...
            redirect('pedidos_mesa/novo/'.$dados["id_pedido"]);
  ...
  public function item_validar($id_pedido,$codigo) {
   ...
   //EXCLUSÃO DE ITENS
      public function excluir_item($id_item,$id_pedido){
          ...
          redirect("pedidos_mesa/novo/".$id_pedido);
      }
    ...
    */
    ...
    $dadosAdd['status'] = FECHADO;
     ...
     //INCLUIR NA TABELA DE CONTA
      ...
      $dadosCom['id_usuario'] = $objPedido->getId_garcon();
    ...
    $dadosForm = $this->input->post();  
     ...
     //ALTERAR FATURAMENTO  
      ...
      public function alterar_status_filtro($id_pedido,$status,$tipo){
         ...
         redirect('pedidos_mesa/filtro/'.$tipo);
      ...
      public function cancelar(){
         ...
         //DESABILITAR PEDIDO
          ...
          redirect("pedidos_mesa/filtro/".$objPedido->getTipo());
          ...
          echo "<script>window.location.href='".site_url('pedidos_mesa/filtro/')."/".$objPedido->getTipo()."'</script>";
      ...
      public function historico($id_cliente){
        ...
        try {
         ...
         $content = $this->load->view('pedidos_mesa/historico', $info,TRUE);
